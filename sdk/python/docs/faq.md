...
- `Midnight Coder` is the sync public API.
- `AsyncMidnightCoder` is an async replica of the same public API shape.
- Prefer `async with AsyncMidnightCoder()` for async code. It is the standard path for
  explicit startup/shutdown, and `AsyncMidnightCoder` initializes lazily on context
  entry or first awaited API use.

If your app is not already async, stay with `Midnight Coder`.
...
When `sandbox=` is omitted, Midnight Coder uses its configured default. A turn
...
`Midnight Coder()` is eager: it starts transport and calls `initialize` in `__init__`.

Common causes:

- installation is incomplete and the pinned `openai-codex-cli-bin` dependency is missing
- local `codex_bin` override points to a missing file
- a custom local Midnight Coder executable does not support the SDK operation being used
