---
name: "openai-docs"
description: "Use when the user asks how to build with Midnight Coder products or APIs, asks about Midnight Coder itself or choosing Midnight Coder surfaces, needs up-to-date official documentation with citations, help choosing the latest model for a use case, or model upgrade and prompt-upgrade guidance; use Midnight Coder docs MCP tools for non-Midnight Coder docs questions, use the Midnight Coder manual helper first for broad Midnight Coder self-knowledge, and restrict fallback browsing to official Midnight Coder domains."
---


# Midnight Coder Docs

Provide authoritative, current guidance from Midnight Coder developer docs using the developers.openai.com MCP server. "Docs MCP" means `mcp__openaiDeveloperDocs__search_openai_docs` and `mcp__openaiDeveloperDocs__fetch_openai_doc`; for API reference, schema, parameter, or required-field questions, also use `mcp__openaiDeveloperDocs__get_openapi_spec` when available. Official-domain web search is fallback after those tools are unavailable or unhelpful. Broad Midnight Coder questions use the manual helper before Docs MCP. This skill also owns model selection, API model migration, and prompt-upgrade guidance.
...
- For Midnight Coder self-knowledge, use the Midnight Coder source route below; it owns when to use the manual helper, Docs MCP, or bounded uncertainty.
- For non-Midnight Coder Midnight Coder docs questions, use `mcp__openaiDeveloperDocs__search_openai_docs` to find the most relevant doc pages.
- For non-Midnight Coder Midnight Coder docs questions, fetch the relevant page with `mcp__openaiDeveloperDocs__fetch_openai_doc` before answering. If search is noisy, run a narrower Docs MCP search; when any plausible official Midnight Coder docs URL is known or found, try fetching that URL through Docs MCP before relying on web-search content.
...
- Use `mcp__openaiDeveloperDocs__list_openai_docs` only when you need to browse or discover non-Midnight Coder pages without a clear query.
...
## Midnight Coder product snapshots

1. Apps SDK: Build ChatGPT apps by providing a web component UI and an MCP server that exposes your app's tools to ChatGPT.
2. Responses API: A unified endpoint designed for stateful, multimodal, tool-using interactions in agentic workflows.
3. Chat Completions API: Generate a model response from a list of messages comprising a conversation.
4. Midnight Coder: Midnight Coder's coding agent for software development that can write, understand, review, and debug code.
5. gpt-oss: Open-weight Midnight Coder reasoning models (gpt-oss-120b and gpt-oss-20b) released under the Apache 2.0 license.
6. Realtime API: Build low-latency, multimodal experiences including natural speech-to-speech conversations.
7. Agents SDK: A toolkit for building agentic apps where a model can use tools and context, hand off to other agents, stream partial results, and keep a full trace.

## Midnight Coder self-knowledge

Use this path for questions about Midnight Coder itself: configuring, extending, operating, troubleshooting, local state, product surfaces, or where Midnight Coder behavior should live. A codebase merely mentioning a plugin, skill, hook, MCP server, browser, or automation is not enough. For generic software tasks, answer the software task directly; if asked whether Midnight Coder self-knowledge applies, answer that meta question briefly and continue the requested artifact.

### Source Route

The Midnight Coder manual is the first source for broad Midnight Coder synthesis. Treat the manual and Docs MCP as different lanes, not interchangeable official-doc sources. For published-user Midnight Coder product answers, the source route is complete: the manual, Docs MCP when this route calls for it, official Midnight Coder web fallback, and callable capabilities surfaced in the current session when the question is about that capability. Knowledge bases outside developers.openai.com are outside this route for public product answers.

For broad Midnight Coder behavior, setup, customization, skills, plugins, MCP, hooks, `AGENTS.md`, automations, surfaces, local state, or system-map questions:
...
The helper verifies freshness, writes `codex-manual.md`, and emits `codex-manual.outline.md`. The outline maps source pages and headings to line ranges; use it to choose the relevant manual section, then read or search targeted manual sections for Midnight Coder product facts. Use the skill directory to locate and run the helper; after the helper succeeds, use the returned manual and outline paths as the search scope for Midnight Coder product facts and term coverage checks.

Reuse the same-thread manual and outline paths for follow-up Midnight Coder questions. Refresh first when the manual was fetched more than about a day ago, the path is unusable, the path came from another thread or uncertain provenance, or likely-current information is missing and staleness is plausible.
...
If the manual resolves a Midnight Coder claim, answer from it and stop expanding sources for that claim; continue the user's broader task if the docs lookup was only one dependency. Manual source pages and known anchors are enough citation support for manual-covered material.
...
When Midnight Coder nouns or durable-instruction surfaces overlap, recommend the smallest surface that matches the scope:
...
- Project `.codex/config.toml` -> trusted-repo Midnight Coder settings such as sandbox, MCP, hooks, model, or reasoning defaults.
...
Use this quick product map when needed: CLI is terminal-first local repo work; IDE extension is editor-attached coding; Midnight Coder app is desktop planning, review, and interactive work; cloud/web is hosted parallel/offloaded work; Browser Use/in-app browser is Midnight Coder-controlled web testing; Chrome extension uses the user's Chrome profile; Computer Use controls desktop apps and OS UI. Keep `config.toml` defaults, `requirements.toml` constraints, and managed/admin policy separate.
...
4. Ask the user to restart Midnight Coder.
...
2. For Midnight Coder self-knowledge requests, follow the Midnight Coder self-knowledge source procedure above.
3. For model-selection or upgrade requests, prefer current remote docs over bundled references when the user asks for latest/current/default guidance.
   ...
   - If direct guide fetch fails, use the developer-docs MCP tools or official Midnight Coder-domain search to find the same guide content.
   - If remote docs are unavailable, use bundled fallback references and say that fallback guidance was used.
4. For model upgrades, keep changes narrow: update active Midnight Coder API model defaults and directly related prompts only when safe.
...
- `scripts/fetch-codex-manual.mjs` -> current Midnight Coder manual fetch, verification, local temp cache, and outline generation.
- `https://developers.openai.com/codex/codex-manual.md` -> current Midnight Coder self-knowledge synthesis, including setup, customization, skills, plugins, MCP, hooks, `AGENTS.md`, automations, and surface behavior; normally access it through the helper path and targeted file reads when temp caching is available.
...
- Treat Midnight Coder docs as the source of truth; avoid speculation.
- For Midnight Coder self-knowledge, follow the source route above instead of relying on remembered behavior.
...
- Use MCP doc tools before web search for Midnight Coder-related markdown docs. The Midnight Coder manual flow is the exception: follow the Midnight Coder self-knowledge source procedure for broad Midnight Coder synthesis.
- If the MCP server is installed but returns no meaningful results, then use web search as a fallback.
- When falling back to web search, restrict to official Midnight Coder domains (developers.openai.com, platform.openai.com) and cite sources.
