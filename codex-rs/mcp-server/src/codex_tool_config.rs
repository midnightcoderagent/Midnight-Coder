...
pub struct MidnightCoderToolCallParam {
    /// The *initial user prompt* to start the MidnightCoder conversation.
    pub prompt: String,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub approval_policy: Option<MidnightCoderToolCallApprovalPolicy>,

    /// Sandbox mode: `read-only`, `workspace-write`, or `danger-full-access`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub sandbox: Option<MidnightCoderToolCallSandboxMode>,
...
pub enum MidnightCoderToolCallApprovalPolicy {
    Untrusted,
    OnRequest,
    Never,
}

impl From<MidnightCoderToolCallApprovalPolicy> for AskForApproval {
    fn from(value: MidnightCoderToolCallApprovalPolicy) -> Self {
        match value {
            MidnightCoderToolCallApprovalPolicy::Untrusted => AskForApproval::UnlessTrusted,
            MidnightCoderToolCallApprovalPolicy::OnRequest => AskForApproval::OnRequest,
            MidnightCoderToolCallApprovalPolicy::Never => AskForApproval::Never,
        }
    }
}
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq)]
#[serde(rename_all = "kebab-case")]
pub enum MidnightCoderToolCallSandboxMode {
    ReadOnly,
    WorkspaceWrite,
    DangerFullAccess,
}

impl From<MidnightCoderToolCallSandboxMode> for SandboxMode {
    fn from(value: MidnightCoderToolCallSandboxMode) -> Self {
        match value {
            MidnightCoderToolCallSandboxMode::ReadOnly => SandboxMode::ReadOnly,
            MidnightCoderToolCallSandboxMode::WorkspaceWrite => SandboxMode::WorkspaceWrite,
            MidnightCoderToolCallSandboxMode::DangerFullAccess => SandboxMode::DangerFullAccess,
        }
    }
}

/// Builds a `Tool` definition (JSON schema etc.) for the MidnightCoder tool-call.
pub(crate) fn create_tool_for_codex_tool_call_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        ...
        .into_generator()
        .into_root_schema_for::<MidnightCoderToolCallParam>();

    let input_schema =
        create_tool_input_schema(schema, "MidnightCoder tool schema should serialize");

    Tool::new(
        "codex",
        "Run a MidnightCoder session. Accepts configuration parameters matching the MidnightCoder Config struct.",
        input_schema,
    )
...
impl MidnightCoderToolCallParam {
    /// Returns the initial user prompt to start the MidnightCoder conversation and the
...
pub struct MidnightCoderToolCallReplyParam {
    /// DEPRECATED: use threadId instead.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    conversation_id: Option<String>,

    /// The thread id for this MidnightCoder session.
    ...
    /// The *next user prompt* to continue the MidnightCoder conversation.
    pub prompt: String,
}

impl MidnightCoderToolCallReplyParam {
...
pub(crate) fn create_tool_for_codex_tool_call_reply_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        ...
        .into_generator()
        .into_root_schema_for::<MidnightCoderToolCallReplyParam>();

    let input_schema =
        create_tool_input_schema(schema, "MidnightCoder reply tool schema should serialize");

    Tool::new(
        "codex-reply",
        "Continue a MidnightCoder conversation by providing the thread id and prompt.",
        input_schema,
    )
    .with_title("MidnightCoder Reply")
    .with_raw_output_schema(codex_tool_output_schema())
}
...
mod tests {
    ...
    fn verify_codex_tool_json_schema() {
        ...
        let expected_tool_json = serde_json::json!({
          "description": "Run a MidnightCoder session. Accepts configuration parameters matching the MidnightCoder Config struct.",
          "inputSchema": {
            ...
            "properties": {
              ...
              "prompt": {
                "description": "The *initial user prompt* to start the MidnightCoder conversation.",
                "type": "string"
              },
              "sandbox": {
    ...
    fn codex_tool_call_param_rejects_removed_profile_field() {
        let err = serde_json::from_value::<MidnightCoderToolCallParam>(serde_json::json!({
            "prompt": "hello",
            "profile": "work"
        }))
    ...
    fn verify_codex_tool_reply_json_schema() {
        ...
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "description": "Continue a MidnightCoder conversation by providing the thread id and prompt.",
          "inputSchema": {
            "properties": {
              "conversationId": {
              ...
              "prompt": {
                "description": "The *next user prompt* to continue the MidnightCoder conversation.",
                "type": "string"
              },
              "threadId": {
                "description": "The thread id for this MidnightCoder session. This field is required, but we keep it optional here for backward compatibility for clients that still use conversationId.",
                "type": "string"
              }
            },
            "required": [
